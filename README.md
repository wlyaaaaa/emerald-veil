...
所有 Windows 屏幕（包括 VDD）的底层 Windows 壁纸与系统锁屏统一使用 **雨林黑猫 · 更绿版 4K 图片**，并保留 Wallpaper Engine。运行 `scripts/Set-WindowsBackground.ps1 -Action Apply`，再用 `-Action Verify` 逐屏核对；换机使用相同入口。素材、来源与操作说明见 [Windows 壁纸与锁屏恢复](docs/windows-background.md)。这套静态背景独立于泡泡程序；泡泡启动时优先复用 VDD 当前 Wallpaper Engine 材料，没有可用材料时才回退到 VDD 的 Windows 壁纸。

Emerald Veil is a small, reversible Windows-native Bubbles overlay for OLED idle use. After six minutes without meaningful keyboard or mouse input, it shows the copy of `Bubbles.scr` already supplied by Windows above the VDD-selected background snapshot. The original Microsoft colors, glass material, size, count policy, and motion stay intact.

The static Windows desktop/lock-screen image is managed separately by `Set-WindowsBackground.ps1`; it is not the screen-saver background contract. When Emerald Veil starts Bubbles, it first composes the material already selected on VDD: use the existing Wallpaper Engine VDD material/properties when a real VDD renderer selection is available, otherwise use VDD's Windows wallpaper/color/fit. Native Bubbles then takes its normal entry snapshot. The watchdog does not continuously capture the VDD desktop and does not move the VDD's ordinary windows onto the physical screen.

The watchdog composes that selected background once before native initialization and never globally stops, pauses, replays or reconfigures Wallpaper Engine. Failures use bounded cooldown and suspension rather than repeated wallpaper takeovers. Only positively identified physical desktop screens are eligible; VDD, instrument screens and unknown identities are excluded. With the physical screen off, the enabled app keeps measuring idle but draws nothing. Windows runtime timeout/active/secure settings are checked every 30 seconds.
...
- Foreground: Codex and other applications remain live and receive normal input; while the veil is active, the selected VDD background material covers the desktop visually beneath the bubbles.
...
状态命令不会启动缺失的屏保；重复打开已运行的程序不会强制预览。`Ctrl+Win+E` 使用现有低级键盘观察器触发，不新建服务或第二套全局热键注册；组合键本身会被消费，所有按键释放并稳定约 250 ms 后才建立屏保输入基线，避免松键把刚启动的屏保立即退出。预览最多 15 秒；真正屏保一直显示到正常输入或禁用。实体屏动画、真实远程画面和自然开机属于单独验收，不从进程存在推断。
